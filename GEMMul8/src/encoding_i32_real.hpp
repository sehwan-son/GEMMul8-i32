...
__global__ void encode_A_i32_kernel(
    ...
    for (unsigned mod_idx = 0; mod_idx < num_moduli; ++mod_idx) {
        A8i[mod_idx * incA8i + out_idx] = mod_centered_i8(in, get_modulus(mod_idx));
    }
}
...
__global__ void encode_B_i32_kernel(
    ...
    for (unsigned mod_idx = 0; mod_idx < num_moduli; ++mod_idx) {
        B8i[mod_idx * incB8i + out_idx] = mod_centered_i8(in, get_modulus(mod_idx));
    }
}
...
__forceinline__ void encode(
    ...
    encode_A_i32_kernel<<<gridA, threads, 0, stream>>>(op_A, m, k, num_moduli, A, lda, A8i, lda8i, incA8i, cola8i);
    encode_B_i32_kernel<<<gridB, threads, 0, stream>>>(op_B, n, k, num_moduli, B, ldb, B8i, ldb8i, incB8i);
}
